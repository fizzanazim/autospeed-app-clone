...
const Orders = () => {
    ...
    const handleorderstatus = (e: any, ele: any) => {
        // Optionally, update order status for a specific order
        setOrderstatus({
            orderEmail: ele.email,
    ...
    useEffect(() => {
        ...
        const updateorderstatus = async () => {
            await axios.patch('http://localhost:2005/api/update-order-status', orderstatus);
        };
                        ...
                        {order_info && order_info.map((ele: any, i: any) => (
                            <div key={i} className='flex justify-between p-5 border'>
                                <div className='flex flex-col gap-3 w-1/2 px-10'>
                                    <h2><span className='font-semibold'>Order ID:</span> {ele.order_id}</h2>
                                    ...
                                    <select className='h-10 border px-2 w-1/3'
                                        name=""
                                        id="orderstatus"
                                        value={ele.order_status}
                                        onChange={(e) => handleorderstatus(e, ele)}
                                    >
